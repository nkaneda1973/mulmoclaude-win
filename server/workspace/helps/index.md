...
- [Custom views](config/helps/custom-view.md) — give a collection a bespoke view the built-ins don't cover (year/quarter overview, Gantt, report): author an HTML file under `views/`, register it in `schema.json` `views[]`, read/write records through the sandboxed `window.__MC_VIEW` data contract
