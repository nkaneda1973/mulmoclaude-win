...
const jaMessages = {
  ...
  collectionsView: {
    ...
    addFeedPrompt:
      "次の URL からデータソースフィードを追加してください: {url}\n\nまず `config/helps/feeds.md` を読み、その指示に正確に従ってください。その URL を自分で取得してレスポンスを調べ、ふさわしいタイトルとフィールドを推測し、ヘルプの説明どおりに `feeds/<slug>/schema.json` を作成してください。私には質問せず、データからすべて判断してください。取得はホストがスケジュールに従って行います。完了したらフィードを登録できたことを教えてください。",
    title: "コレクション",
