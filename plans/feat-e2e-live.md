...
#### L-31: 「manageSkills tool で skill を作って」 と明示すると Claude が manageSkills を呼ぶ (canary)
...
- フォローアップメモ: issue #1290（manageSkills を General ロールに戻す）が merge されたら、 Tutor 経由ではなく **General で動かす形に変更する**（ユーザー実面に近い）。 同時に L-32 を「曖昧プロンプトで skill が確実に landing する」 outcome ベースの canary として再実装する
