...
| L-17 | ui | notifier engine publish はベルバッジだけを更新し session-history unread badge は変えない (B-50) | ✅ |
...
#### L-17: notifier engine publish はベルバッジだけを更新し session-history unread badge は変えない (B-50) — ✅ 実装済

- カバー: B-50
- 重要度: **B** / Docker: `both` / 画像: 不要
- 元シナリオ: bridge から外部メッセージを送信 → 通知 bell バッジは更新されず、history バッジのみ更新
- 実装上の整形: bridge メッセージを e2e から直接走らせるには WebSocket 接続 + real LLM が必要で実用的でなかったため、 同じ B-50 不変条件を「notifier engine publish 経路から session.hasUnread に副作用が出ない」 という形で観測する canary に置き換えた。 (a) `server/api/routes/notifier.ts` の `publish` action を `MULMOCLAUDE_E2E_NOTIFIER_INJECT=1` env-gate 付きで復活 (PR #818 で agent.ts から publishNotification を撤去した後、 `00f4a740` で HTTP publish 自体も落としていたため復活が必要だった)、 (b) `e2e-live/tests/ui.spec.ts` に L-17 spec を追加: 初期 unread badge を snapshot → inject 1 件 → 当該 row が bell panel に visible → unread badge snapshot が変わっていないことを assert → finally で clear
...
- env-gate: 開発 / 本番では publish action は 400 "unknown action"。 `MULMOCLAUDE_E2E_NOTIFIER_INJECT=1` を set した dev server でのみ inject 可能。 CI 側は `.github/workflows/e2e_live_no_llm.yaml` の `MULMOCLAUDE_E2E_NOTIFIER_INJECT: "1"` で常時有効
- 残課題: ない (本 spec は env が無ければ skip)。 もし将来 bridge end-to-end も入れたければ「test 専用 dev server spawn infra」 (Phase 2 #4) の上で別 L-XX として作る
...
| **L-17** notifier publish 単独で session-history badge は変わらない (B-50) | ✅ 実装済 | ui.spec.ts、 `MULMOCLAUDE_E2E_NOTIFIER_INJECT=1` で `/api/notifier` の `publish` action を env-gate で復活 → `injectNotifierEntry(page, {...})` で 1 件 inject → bell panel を開いて `notification-item-<id>` が visible (publish が engine に届いた canary) → `[session-history-unread-badge]` の {visible, text} snapshot を before/after で比較し変化していないことを assert (B-50 regression: notifier publish と session.hasUnread が同じ event channel を共有してしまった場合に両方 tick する形を catch) → finally で `clearNotifierEntry(page, id)` 。 bridge を直接走らせる元シナリオは WebSocket + real LLM 必要で実用的でなかったため、 同じ不変条件をより狭く端的に観測する整形を選択。 PR 内で `SessionHistoryToggleButton` の unread / active 両 badge に testid を新設し `docs/ui-cheatsheet.md` に節を追加。 CI 側は `.github/workflows/e2e_live_no_llm.yaml` で env を常時 on |
...
| **L-17** notifier 二重通知 (B-50) | ✅ **実装済** | `server/api/routes/notifier.ts` の `publish` action を `MULMOCLAUDE_E2E_NOTIFIER_INJECT=1` env-gate 付きで復活、 `ui.spec.ts` に inject → 当該 row visible → `[session-history-unread-badge]` 不変 → clear の canary を追加。 bridge end-to-end の代わりに「notifier publish 経路が session.hasUnread に副作用を出さない」 不変条件を観測する形に整形。 PR では `SessionHistoryToggleButton` の unread / active 両 badge に testid を新設し `docs/ui-cheatsheet.md` に節を追加。 CI 側は `.github/workflows/e2e_live_no_llm.yaml` で env 常時 on |
...
- ~~**L-17 用 notifier inject endpoint**~~ ✅ **解消済** — `server/api/routes/notifier.ts` に `MULMOCLAUDE_E2E_NOTIFIER_INJECT=1` で開く `publish` action を追加。 production はデフォルト off (production POST が `publish` を投げると 400 "unknown action")、 e2e CI workflow 側で env 常時 on にしてある
...
| **#1451** notifier-update-op | notifier 拡張、 update operation 追加 | L-17 (二重通知 inject canary) が同 PR で env-gated `publish` action を復活させたので update op を追加するなら同 seam を拡張する形が低コスト。 ただし update op の dispatch を見るには「先に inject → update → 再 inject 」 のシナリオが必要で、 update op 自体の canary は今は不要 (L-17 は B-50 不変条件のみが scope) |
...
- [x] ~~**L-17 用 bridge メッセージ inject 経路の追加**~~: ✅ **解消済**。 採用案 (a) test 専用 inject endpoint を env-gate で復活、 を選択。 `server/api/routes/notifier.ts` に `MULMOCLAUDE_E2E_NOTIFIER_INJECT=1` で開く `publish` action を追加し、 `ui.spec.ts` の L-17 canary が inject → bell row visible → `[session-history-unread-badge]` 不変 を観測。 (b) socket.io 直接 emit、 (c) 実 bridge WebSocket は将来 bridge-end-to-end をやる際の選択肢として残す
